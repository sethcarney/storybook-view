...
export class ComponentParser {
    ...
    private extractComponentName(filePath: string, code: string): string {
        ...
        const functionMatch = code.match(/(?:export\s+)?(?:const|function)\s+(\w+)\s*[=\(]/);
        if (functionMatch) {
    ...
    private extractProps(code: string): Record<string, PropInfo> {
        ...
        if (propTypesMatch) {
            ...
            const propMatches = propsText.matchAll(/(\w+):\s*PropTypes\.(\w+)(?:\.isRequired)?/g);
        ...
        if (Object.keys(props).length === 0) {
            ...
            if (destructuringMatch) {
                ...
                const propNames = propsText.split(',').map(p => p.trim().split(/[=\s]/)[0]);
